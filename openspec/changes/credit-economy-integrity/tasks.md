...
- [ ] 1.1 Create `server/migrations/0014_credit_ledger_integrity.sql`. Spanish `--` prose header in
...
- [ ] 1.2 In `0014`, add to `wallet_transactions`: `balance_delta BIGINT NOT NULL DEFAULT 0`,
...
- [ ] 1.3 In `0014`, back-fill `balance_delta` for existing rows from `positive`/`amount` so the
      admin history at `server/api/src/models/admin.ts:353` is not silently zero.
- [ ] 1.4 In `0014`, add a `CHECK` on `reason` over the sixteen values in design D5, allowing `NULL`
      for legacy rows.
- [ ] 1.5 In `0014`, drop `wallet_transactions_user_community_fk`
...
- [ ] 1.6 In `0014`, insert one `genesis_opening_balance` row per user with
...
- [ ] 1.7 In `0014`, de-duplicate `user_missions` on `(user_id, mission_template_id)` keeping the
...
- [ ] 1.8 In `0014`, de-duplicate `mission_templates` on `key`, then
...
- [ ] 1.9 In `0014`, `ALTER TABLE listings ADD COLUMN IF NOT EXISTS status_changed_at TIMESTAMP(0)`
...
- [ ] 1.10 Apply and verify: `docker compose -f docker-compose.dev.yml up -d db api && npm run dev:migrate`,
...
- [ ] 2.1 Add `queries.applyCreditDelta` to `server/api/src/services/queries.ts` — the guarded
...
- [ ] 2.2 Add `queries.insertCreditLedgerEntry` to `server/api/src/services/queries.ts`, writing
...
- [ ] 2.3 Create `server/api/src/utils/credits.ts` with `CreditReason` (the sixteen values of design
...
- [ ] 2.4 In `applyCreditMovements`, on zero affected rows, re-read via `queries.userById` to
...
- [ ] 2.5 **Delete** `queries.updateUserBalance` (`server/api/src/services/queries.ts:448-453`). The
...
- [ ] 3.1 Add `listing_status = 'published' AND buyer_id IS NULL` plus `RETURNING *` to
...
- [ ] 3.2 Rewrite the buyer lock in `newOffer` (`server/api/src/models/listings.ts:365-370`) as a
      single `applyCreditMovements` call with `offer_lock`. — *credit-ledger: Ledger Entry On Every Movement*
- [ ] 3.3 Rewrite `deleteOffer` (`server/api/src/models/listings.ts:414-419`) → `offer_unlock_withdrawn`,
...
- [ ] 3.4 In `acceptOffer`, replace the unordered `Promise.all` of `getListingById`
...
- [ ] 3.5 Add `listing_status = 'published' AND buyer_id IS NULL AND seller_id = $n` plus `RETURNING *`
...
- [ ] 3.6 Call `queries.storeTrade` (`server/api/src/services/queries.ts:455-459`, currently **zero
      call sites**) once per traded listing inside `acceptOffer`'s transaction. — *listing-lifecycle: "Accepted trades are recorded"*
- [ ] 3.7 Replace both balance writes in `acceptOffer`
...
- [ ] 3.8 Rewrite `receiveListing`'s two writes (`server/api/src/models/listings.ts:724-729`,
...
- [ ] 3.9 Narrow the blanket `catch { throw new InternalServerError(...) }` blocks in
...
- [ ] 3.10 Add `transaction: true` to `createListing`'s options
...
- [ ] 3.11 In `createListing`, validate `price` against `categories.min_price_credits`/
...
- [ ] 4.1 Rewrite `ListingsModel.cancelListing` (`server/api/src/models/listings.ts:824-914`) per
...
- [ ] 4.2 In `cancelListing`, return every listing linked through `listing_trades` to `published`
...
- [ ] 4.3 Emit `cancel_buyer_refund` and `cancel_seller_unlock` through one batched
      `applyCreditMovements`.
- [ ] 4.4 Add `ListingsController.cancelListing` to `server/api/src/controllers/listings.ts`,
      validating `listingId` with `validateId` as its siblings do (`:222`).
- [ ] 4.5 Add `listingsRouter.post("/:listingId/cancel", tokenMiddleware, ListingsController.cancelListing)`
...
- [ ] 5.1 In `SelfModel.deleteSelf` (`server/api/src/models/self.ts:481-522`), before the deletes at
...
- [ ] 5.2 Add `AND listing_status IN ('offered','accepted')` to
...
- [ ] 5.3 Remove the `deleteWalletTransactionsByUserId` call
...
- [ ] 5.4 Collapse `AccountDeletionModel.resolveRequest`
...
- [ ] 6.1 Rewrite `UsersModel.donate` (`server/api/src/models/users.ts:87-98`) as one batched
      `applyCreditMovements` with `donation_sent` and `donation_received`. — *credit-ledger: "A donation records both sides"*
- [ ] 6.2 Add `DONATION_MIN_CREDITS`, `DONATION_MAX_CREDITS`, `DONATION_DAILY_MAX_CREDITS` to
...
- [ ] 6.3 Replace the ad-hoc check at `server/api/src/controllers/users.ts:71`
...
- [ ] 6.4 Add a Zod body schema for `makeOffer` — it currently has **none**
...
- [ ] 6.5 Route `AdminModel.modifyUserCredits` (`server/api/src/models/admin.ts:353-367`) through
...
- [ ] 6.6 Fix `queries.progressMission` (`server/api/src/services/queries.ts:593-598`):
...
- [ ] 6.7 Reorder `progressMission` (`server/api/src/utils/helpersDb.ts:668-691`) so the guarded
...
- [ ] 6.8 Add `WHERE active = true` to `queries.allMissionTemplates`
...
- [ ] 6.9 Replace `assignMissionToAllUsers` (`server/api/src/utils/helpersDb.ts:769-802`, a
...
- [ ] 6.10 In `AdminModel.moveUserToCommunity` (`server/api/src/models/admin.ts:1069`), call
...
- [ ] 7.1 Add `listing_sold` to `NOTIFICATION_TEXTS.LOOP_NOTIFICATION`
...
- [ ] 7.2 In `getSchoolsByIds` (`server/api/src/utils/helpersDb.ts:130-137`), return the school with a
      null media reference instead of dropping it. — *notification-integrity: "A school with unreachable media is still listed"*
- [ ] 7.3 In `getNotificationsByUserId` (`server/api/src/utils/helpersDb.ts:600-608`), stop discarding
...
- [ ] 8.1 Create `server/api/src/scripts/reconcileCredits.ts` running the design D6 query under
...
- [ ] 8.2 Add `"reconcile-credits": "tsx src/scripts/reconcileCredits.ts"` to
      `server/api/package.json` beside `migrate` (`:20`).
- [ ] 8.3 Create `server/api/src/tests/creditConcurrency.test.ts` following the `rls.test.ts` pattern
...
- [ ] 8.4 Test 1 — two simultaneous offers on one listing, fired on **separate connections** and
...
- [ ] 8.5 Test 2 — one user with balance N issuing two simultaneous donations of N, on separate
...
- [ ] 8.6 Test 3 — run the design D6 reconciliation query over the fixtures after tests 1 and 2 and
      assert zero discrepancies. — *credit-ledger: Reconciliation Invariant*
- [ ] 8.7 Test 4 — lock ordering: one transaction accepting an offer while another donates in the
...
- [ ] 8.8 Run the suite. **Exact commands, from the repository root:**
      ```
      docker compose -f docker-compose.dev.yml up -d db api
      npm run dev:migrate
...
- [ ] 9.1 `cd server/api && npm run check-types` — passes. This is the exhaustiveness proof for
      task 2.5: every removed `updateUserBalance` call site must have been rewritten.
- [ ] 9.2 `cd server/api && npm run check-sql` — passes. Every new query's `$N` arity must match its
      call sites (`server/scripts/check-sql-arity.py`).
- [ ] 9.3 `rg "updateUserBalance" server/api/src` returns nothing. — *credit-ledger: Single Mutation Choke Point*
- [ ] 9.4 `cd server/api && npm run reconcile-credits` against the seeded dev database — zero
      discrepancies.
- [ ] 9.5 Playwright: `npm run test:e2e` from the root. `e2e/tests/02_listing_journey.e2e.spec.ts`
...
- [ ] 9.6 Manual readback: confirm ECO-07 is unchanged — a listing priced 100, an offer of 80 with no
...
- [ ] 9.7 Update `AUDITORIA-PROGRESO.md`: mark block C closed, list ECO-07 under "Salteados a
