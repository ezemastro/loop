...
- [x] 2.2 [RED→GREEN] Add a column-math unit test (in `responsive-tokens.test.ts` or a new `listing-grid.test.ts`): `columnsAt()` returns 1/2/3/4 per breakpoint, and `GRID_CELL_CLASS` contains exactly the matching `w-full`/`w-1/2`/`w-1/3`/`w-1/4` fraction tokens. — *listing-discovery-grid: "Column count per range"*
- [x] 2.3 Modify `client/components/CategoryBadge.tsx`: add an optional `numberOfLines?: number` passthrough (default behavior unchanged).
- [x] 2.4 Rewrite `client/components/cards/Listing.tsx` to the image-first markup from design D3: `Image` `aspect-[4/3] w-full bg-background` hero with `resizeMode="cover"`, badge/content block below reusing `CreditsBadge`/`CategoryBadge`/`ProductStatusBadge`/`UserBadge` unchanged, zero variant props. — *listing-discovery-grid: Image-First Card Variant*
...
- [x] 2.5 Apply `ELEVATION.raised` to the card container (D1): `twMerge` the web `class` on web, apply the native `style` object on native via `Platform.select`.
- [x] 2.6 Modify `client/components/Feed.tsx`: wrap the listing `.map()` output in `<ListingGrid>`; move `Loader`/`Error`/"Cargar más" outside the grid container (required — `ListingGrid` wraps every child in a cell). — *listing-discovery-grid: Grid Column Count by Breakpoint*
- [x] 2.7 Modify `client/components/bases/ListingViewList.tsx`: same `ListingGrid` wrap and `Loader`/`Error` relocation as 2.6.
- [x] 2.8 Modify `client/components/screens/Search.tsx`: keep the `FlatList`; set `className="px-4"` on the list, `contentContainerClassName={GRID_ROW_CLASS}`, `renderItem={({item}) => <View className={GRID_CELL_CLASS}><Listing listing={item}/></View>}`. — *listing-discovery-grid: Grid Column Count by Breakpoint (FlatList path)*
  - **Deviation**: kept `className="flex-1 px-4"` instead of the literal `"px-4"` — the pre-existing `flex-1` is required for the `FlatList` to fill its flex-1 `MainView` parent and scroll; dropping it would break scroll/height, not the grid contract. `px-4` was added exactly as specified.
- [x] 2.9 Visual readback at 500/900/1100/1400px on `Home`, `Search`, `MyListings`, `WishList`: confirm 1/2/3/4 columns respectively; resize 1100→1400px and confirm no remount and no lost scroll position. — *listing-discovery-grid: "Column count per range", "Grid reflows without remount or scroll loss"*
  - **Not executed** — no browser/Expo-web runtime available in this environment; this is a manual visual check the orchestrator/user must perform. **Deviation noted**: `WishList.tsx` does not render `Listing`/`ListingGrid` at all (it renders a separate `cards/Wish` component via a plain `FlatList`, out of design.md's slice-2 File Changes table and out of tasks 2.1–2.8); the column-count check does not apply to it as written. Flagging for the orchestrator rather than silently modifying `WishList.tsx`/`Wish.tsx`, which are out of this slice's assigned scope.
- [x] 2.10 Run `cd client && npx jest --ci --watchAll=false`; confirm baseline + slice-1 + slice-2 tests pass.
